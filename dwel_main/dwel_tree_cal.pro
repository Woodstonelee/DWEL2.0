...
; Get the Mask, Zenith, Azimuth from the ancillary file
  ...
  Zenith = float(envi_get_data(fid=anc_fid, dims=dims, pos=7))/100.0
  Azimuth = float(envi_get_data(fid=anc_fid, dims=dims, pos=8))/100.0
...
;first find the extrainfo file for mask zenith and azimuth information
  ...
  zenith = float(envi_get_data(fid=anc_fid, dims=dims, pos=1))/100.0
  azimuth = float(envi_get_data(fid=anc_fid, dims=dims, pos=2))/100.0
...
for i=0,num_rois-1 do begin
  ...
  sinth[i]=sin(!dtor*mean_roi[1]/100.0)
;
  outstring=strtrim(roi_names[i],2)+','+$
      strtrim(string(mean_roi[0],format='(f10.2)'),2)+','+$
      strtrim(string(mean_roi[1]/100.0,format='(f10.2)'),2)+','+$
      strtrim(string(sinth[i],format='(f10.4)'),2)+','+$
      strtrim(string(mean_roi[2]/100.0,format='(f10.2)'),2)+','+$
...
;
  ...
  r_crit=r_max
  if (iphase) then phase_t=phase_trunk(width[i],mean_tree_dbh,r_max,r_crit) else phase_t=1.0
