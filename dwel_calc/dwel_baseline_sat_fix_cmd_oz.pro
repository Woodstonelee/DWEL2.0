...
function apply_sat_fix_oz, basefixed_satwf, pulse_model, i_val, satfixedwf=satfixedwf
...
;prevent overflow values if possible
  nu_pos=where(abs(temp) gt 3276,nu_npos)
...
pro DWEL_Baseline_Sat_Fix_Cmd_oz, DWELCubeFile,ancillaryfile_name,out_satfix_name,Casing_Range,get_info_stats,zen_tweak,err
...
;distance from casing (edge of casing) to the true Tzero position at mirror
  casing2Tzero = 0.065 ; unit=metres
...
;=================================================================================================
  ...
  DWEL_base_fix_info=[$
  'Program=dwel_baseline_sat_fix_cmd_oz',$
...
;============================================
    ...
    satflag=1
      ...
      for si=0,count_sat-1 do begin
        ...
        satflag = apply_sat_fix_oz(reform(temp[sat_pos[si], *]), pulse_model, i_val, satfixedwf=unsattemp)
...
envi_open_file,ancfile,r_fid=anc_fid,/no_interactive_query,/no_realize
  ...
  free_lun,ctfile,/force
  free_lun,osatfile,/force
  if (err ne 0) then print,'Returning from DWEL_Baseline_Sat_Fix_Cmd_oz with error'
  heap_gc,/verbose
;
  return
end
